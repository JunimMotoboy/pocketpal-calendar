    ...
    <div className="space-y-3">
      {monthInstallments.length > 0 && (
        <div className="flex items-center justify-between gap-2">
          <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">
            {monthInstallments.length} parcela{monthInstallments.length === 1 ? "" : "s"} · {pendingCount} pendente
            {pendingCount === 1 ? "" : "s"}
          </p>
          {pendingCount > 0 && (
            <Button size="sm" variant="outline" className="h-7 text-[11px]" onClick={() => onMarkAllPaid(card.id)}>
            ...
            return (
              <li key={`p-${i.id}`} className="flex items-center gap-3 rounded-xl border border-border/60 bg-background p-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <p className={`truncate text-xs font-semibold ${paid ? "text-muted-foreground line-through" : ""}`}>
                    {i.description}
                  </p>
                  <p className="text-[10px] text-muted-foreground">vence dia {card.due_day}</p>
                </div>
                <span className={`text-xs font-bold tabular-nums ${paid ? "text-muted-foreground line-through" : ""}`}>
      ...
      <div className="rounded-lg border border-border/60 p-3">
        <div className="mb-2 flex items-center justify-between">
          ...
          <Button size="sm" variant="ghost" className="h-7 px-2" onClick={() => onNewInstallment(card.id)}>
            ...
            {cardInstallments.map((i) => {
              ...
              return (
                <li key={i.id} className="flex items-center justify-between gap-2 text-xs">
                  ...
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onEditInstallment(i)} aria-label="Editar">
                    <Pencil className="h-3.5 w-3.5 text-muted-foreground" />
                  </Button>
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => onDeleteInstallment(i)} aria-label="Remover">
